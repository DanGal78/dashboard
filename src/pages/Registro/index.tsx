...
const schema = yup.object().shape({
    ...
    .min("2023-04-14", ({min})=> {
        ...
        return `Seu nascimento precisa ser maior que: ${formataData.format(new Date(min+"T03:00:00.000"))}`;
    })
    .typeError("A data informada é inválida")
    .required("Informe sua data de nascimeto"),
...
const Registro: React.FC = () =>{
    ...
    const { register,handleSubmit,formState: {errors} } = useForm<CadastreFromData>({ resolver:yupResolver(schema)});
    const onSubmit= (data:CadastreFromData) =>{
        console.log(data);
    }
    ...
    return(
        ...
        <Input label="Data de nascimento" type="date" id="nascimento"
         {...register("nascimento")} 
        ...
        <Input label="Confirme a senha" type="password" id="confirma-senha" 
        {...register("confirmaSenha")}
         error={errors?.confirmaSenha?.message} />
            ...
            <p>Já possue conta? <Link to="/login">Faça seu login</Link></p>
        </div>  
